...
def generate_raport(user_id, filename, months):
    ...
    if months == 1:
        ...
        czas = "three last months"
    elif months == 6:
        czas = "six last months"
    elif months == 12:
        czas = "twelve last months"
    textobject.textLine(text=f"Report for the {czas}")
    ...
    textobject.textLine(text=f"For the {czas}: ")
    textobject.moveCursor(15, 5)
    textobject.textLines(f'''
        - income amounted to {suma_avg_wplywow[0]} zł,
        - expenses amounted to {suma_avg_wydatkow[0]} zł,
        - balance amounted to {bilans} zł.''')
    if months > 1:
        textobject.moveCursor(-15, 5)
        textobject.textLine(text=f"Which gives an average per month:")
        textobject.moveCursor(15, 5)
        textobject.textLines(f'''
            - income - {round(suma_avg_wplywow[1],2)} zł,
            - expenses - {round(suma_avg_wydatkow[1],2)} zł,
            - balance - {round(bilans/months,2)} zł.''')
    ...
    textobject.textLine(text="Expenses in individual categories amounted to: ")
    textobject.moveCursor(15, 5)
    ...
    textobject.textLine(text=f"The biggest expenses were for {max_category}.")
    c.drawText(textobject)

    if months == 12:
        add_plot(user_id)
        c.drawImage('plot.png', 50, 100, width=500, height=300)
    
    c.showPage()

    c.save()

generate_raport(11, 'report1.pdf', 1)
generate_raport(11, 'report3.pdf', 3)
generate_raport(11, 'report6.pdf', 6)
generate_raport(11, 'report12.pdf', 12)

data = date.today()
nazwa_pdf = f"report-{data}"
merger = PdfWriter()
for pdf in [f'report1.pdf', f'report3.pdf', f'report6.pdf', f'report12.pdf']:
    merger.append(pdf)
merger.write(nazwa_pdf)
merger.close()

os.remove('plot.png')
os.remove(f'report1.pdf')
os.remove(f'report3.pdf')
os.remove(f'report6.pdf')
os.remove(f'report12.pdf')
